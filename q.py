...
class InspireHEPContext(ModelContextProtocol):
    ...
    def search_inspirehep(self, query: str, size: int = 10) -> List[Dict[str, Any]]:
        ...
        data = resp.json()
        play = data.get('hits', {}).get('hits', [])
        # print(play[0]['metadata']['abstracts'][0]['value'])
        return play
...
if __name__ == '__main__':
    ...
    records = hep.search_inspirehep('Quiver', size=5)
    hep.download_pdfs(records)
    print("Downloaded files:", hep.list_downloaded())
    info1 = hep.get_paper_info(records[1])
    print("Sample paper info:", info1)
